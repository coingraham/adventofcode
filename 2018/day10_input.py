real = """position=< 21188,  31669> velocity=<-2, -3>
...
stars_text = real.splitlines()
spaces = [i.split(", ") for i in stars_text]
x = [ int(j[0].split("<")[1]) for j in spaces ]
y = [ int(j[1].split(">")[0]) for j in spaces ]
vel_x = [ int(j[1].split("<")[1]) for j in spaces ]
vel_y = [ int(j[2].split(">")[0]) for j in spaces ]
stars_info = []
velocity_list = []
for item in range(len(stars_text)):
    info = [x[item], y[item]]
    vel = [vel_x[item], vel_y[item]]
    stars_info.append(info)
    velocity_list.append(vel)
    # print("{}: {} {}".format(stars_text[item], stars_info[item], velocity_list[item]))
