...
for i in formulas:
    ...
    a_list = a.split(", ")
    many, name = b.split(" ")
    reactions[name] = {"mod": int(many)}

    for j in a_list:
        m, n = j.split(" ")
        reactions[name][n] = int(m)
...
while stack:
    ...
    current_mod = reactions[current]["mod"]
    current_need = resource_totals[current]
    if current in leftovers:
        current_left = leftovers[current]
    else:
        leftovers[current], current_left = 0, 0
    current_need -= current_left
    children = [k for k in reactions[current].keys() if "mod" not in k]

    if "ORE" not in children:
        for child in children:
            child_mod = reactions[current][child]
            if current_need > current_mod:
                adjustment = ceil(current_need / current_mod)
                left = (adjustment * current_mod) - current_need
                total_need = adjustment * child_mod
            else:
                left = current_mod - current_need
                total_need = child_mod
            if child in leftovers:
                need_adjustment = leftovers[child]
                if need_adjustment > total_need:
                    total_need = 0
                    leftovers[child] -= total_need
                else:
                    total_need -= need_adjustment
                    leftovers[child] = 0
            resource_totals[child] += total_need
            leftovers[current] += left
            if child not in stack:
                stack.append(child)

        del resource_totals[current]

    else:
        needed = resource_totals[current]
        any_left = leftovers.get(current, 0)
        needed -= any_left
        leftovers[current] = 0
        if needed == 0:
            continue
        mod = reactions[current]["mod"]
        ore = reactions[current]["ORE"]
        if needed > mod:
            ore_adj = ceil(needed / mod)
            more = (ore_adj * ore)
            ore_required += more
            left = (ore_adj * mod) - needed
            leftovers[current] += left
            resource_totals[current] = 0
            print("{}: {}".format(current, more))
            print(leftovers)
        else:
            ore_required += ore
            leftovers[current] += mod - needed
            resource_totals[current] = 0
            print("{}: {}".format(current, ore))
            print(leftovers)
