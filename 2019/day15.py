...
def get_paths(pos):
    ...
    for k, v in movement.items():
        if room[pos + v] == ".":
            options.append(pos + v)
        if room[pos + v] == "O":
            options.append(pos + v)

    # return [pos + v for k, v in movement.items() if room[pos + v] == "." or room[pos + v] == "2"]
    return options


def print_screen(pos):
    hold = room[pos]
    room[pos] = "X"
    ac.screen_complex(room)
    room[pos] = hold
...
#     if image % 100000 == 0:
#         print_screen(pos)
#     result = next(computer)
#     proposed_pos = pos + movement[direction]
#     room[proposed_pos] = legend[result]
#
#     if result == 0:
...
#         my_input = directions[direction]
#
#     for i, move in movement.items():
#         to_search.append(pos + move)
#
#     for k in room.keys():
#         if k in to_search:
#             to_search.remove(k)
...
# print_screen(0 + 0j)
#
# with open("room_map", "wb") as f:
#     pickle.dump(room, f)

with open("room_map", "rb") as f:
    room = pickle.load(f)
...
#         value_map[pos] = steps
#         for path in get_paths(pos):
#             if path not in value_map:
#                 explored.append(path)
#
# ac.screen_complex(value_map)
# print(value_map[oxy])

# Part Two run it backwards and count the steps
current = (-16-12j)
value_map = {current: 0}
explored = deque([current])
steps = -1
